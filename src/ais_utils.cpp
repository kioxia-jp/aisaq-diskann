    ...
    ais_read_nodes_nbrs_func_t<T, LabelT> read_nodes_nbrs_func, void *context)
{
    ...
    if (rearranged_vectors_map == nullptr) {
        diskann::cerr << "memory allocation failure" << std::endl;
        return -1;
    }
    /* init */
    ...
    while (cur_level->size() != 0) {
        ...
        for (size_t block = 0; block < nblocks; block++) {
            ...
            for (uint32_t i = 0; i < read_status.size(); i++) {
                if (read_status[i] == false) {
                    diskann::cerr << "read failed" << std::endl;
                    continue;
                }
        ...
        rnodes.clear();
        diskann::cout << "... +" << vid_hover - prev_vid_hover << " vectors"
                      << " --> " << vid_hover << " vectors" << std::endl;
...
{
    if (!file_exists(rearranged_vectors_map_path)) {
        diskann::cerr << "file not found " << rearranged_vectors_map_path << std::endl;
        return -1;
    }
    /* allocate memory */
    rearranged_vectors_map = new uint32_t[num_points];
    ...
    do {
        ...
        diskann::cout << "loading vectors rearrange mapping data from " << rearranged_vectors_map_path << "..." << std::flush;
        std::ifstream rearranged_vectors_reader;
        uint64_t file_size = get_file_size(rearranged_vectors_map_path);
        rearranged_vectors_reader.exceptions(std::ofstream::failbit | std::ofstream::badbit);
        try {
            uint32_t rearranged_vectors_npts, vector_id_nbytes;
            rearranged_vectors_reader.open(rearranged_vectors_map_path, std::ios::binary);
            rearranged_vectors_reader.read((char *) &rearranged_vectors_npts, sizeof(uint32_t));
            rearranged_vectors_reader.read((char *) &vector_id_nbytes, sizeof(uint32_t));
            if (vector_id_nbytes != sizeof(uint32_t) ||
                rearranged_vectors_npts != num_points ||
                file_size != 8 + (size_t) rearranged_vectors_npts * (size_t) vector_id_nbytes) {
                file_error = true;
                }
        ...
        if (file_error) {
            ...
            diskann::cerr << "invalid/corrupted vectors rearrange mapping file" << std::endl;
            /* cleanup and exit */
            break;
        }
        uint32_t roffset = 0;
        do {
            uint32_t r = std::min((uint32_t) (128 << 10), (uint32_t) ((num_points * sizeof(uint32_t)) - roffset));
            if (r == 0) {
                break;
            }
            rearranged_vectors_reader.read((char *)rearranged_vectors_map + roffset, r);
            roffset += r;
        } while (true);
        rearranged_vectors_reader.close();
        /* repair the map
           in first rearranged version, unreferenced vectors serialized with AIS_INVALID_VID value
           in newer version we do not allow that */
        uint32_t i, max_vid = 0, unref_count = 0, vid;
        for (i = 0; i < num_points; i++) {
            vid = rearranged_vectors_map[i];
            if (vid == AIS_INVALID_VID) {
                unref_count++;
            } else {
                assert(vid < num_points);
                if (vid > max_vid) {
                    max_vid = vid;
                }
            }
        }
        assert(max_vid + 1 == num_points - unref_count);
        for (i = 0; i < num_points; i++) {
            if (rearranged_vectors_map[i] == AIS_INVALID_VID) {
                rearranged_vectors_map[i]=++max_vid;
            }
        }
...
int ais_save_vectors_rearrange_map(uint32_t *rearranged_vectors_map,
    const std::string &rearranged_vectors_map_path, uint32_t num_points)
{
    assert(rearranged_vectors_map != nullptr);
    diskann::cout << "saving vectors rearrange mapping data to file " << rearranged_vectors_map << "..." << std::flush;
    std::ofstream fwriter(rearranged_vectors_map_path, std::ios::binary);
    uint32_t u32val = sizeof(uint32_t);
    fwriter.write((char *) &num_points, sizeof(uint32_t));
    fwriter.write((char *) &u32val, sizeof(uint32_t));
    uint32_t woffset = 0;
    do {
        uint32_t w = std::min((uint32_t) (128 << 10), (uint32_t) ((num_points * sizeof(uint32_t)) - woffset));
        if (w == 0) {
            break;
        }
        fwriter.write((char *)rearranged_vectors_map + woffset, w);
        woffset += w;
    } while (true);
    fwriter.close();
    diskann::cout << "done" << std::endl;
...
int ais_create_reversed_vectors_map(uint32_t *&reversed_vectors_map, const uint32_t *vectors_map, uint32_t num_points)
{
    ...
    if (reversed_vectors_map == nullptr) {
        diskann::cerr << "failed to allocate memory." << std::endl;
        return -1;
    }
    uint32_t i;
    for (i = 0; i < num_points; i++) {
    ...
    if (i < num_points) {
        ...
        diskann::cerr << "rearranged vectors map error" << std::endl;
        return -1;
    }
    return 0;
}
            ...
            uint32_t page_size, uint32_t *rearrange_map, uint32_t num_points, uint32_t pq_vector_size)
{
    ...
    do {
        ...
        uint8_t *page_buff = new uint8_t[page_size];
        ...
        if (file_error) {
            diskann::cout << "...error" << std::endl;
            diskann::cerr << "failed to open/write pq compressed rearranged vectors file" << std::endl;
            break;
        }
        diskann::cout << "...done" << std::endl;
        return 0;
            ...
            uint32_t page_size, uint32_t *rearrange_map, uint32_t num_points, uint32_t pq_vector_size)
{
    ...
    try {
        ...
        diskann::cerr << "failed to open/read header of pq compressed vectors file " << pq_compressed_vectors_path << std::endl;
        return -1;
    }
